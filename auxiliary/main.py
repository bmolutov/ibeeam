...
import database
from models import CreateGetUpdateUserProfile, ListUpdateUserProfile


app = FastAPI()


@app.get('/list_user_profiles', response_description="Get all user profiles", response_model=ListUpdateUserProfile)
...
async def create_user_profile(user_profile: CreateGetUpdateUserProfile = Body(...)):
    ...
    created_user_profile = await database.users_collection.find_one({"_id": new_user_profile.inserted_id})
    return JSONResponse(status_code=status.HTTP_201_CREATED, content=created_user_profile)
